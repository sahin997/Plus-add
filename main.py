...
class NumberFormatterBot:
    ...
    def split_or_file(self, chat_id, text: str, caption: str, filename=None):
        if len(text) <= self.MAX_MSG_CHARS:
            ...
            fname = filename if filename else "numbers.txt"
    ...
    def register_handlers(self):
        ...
        def callback_handler(call):
            ...
            if call.data == "send_numbers":
                self.bot.send_message(chat_id, "Send numbers separated by space, comma or newline.")

            elif call.data == "upload_file":
                self.bot.send_message(chat_id, "Upload .txt, .csv or .xlsx file.")

            elif call.data == "how_to_use":
                self.bot.send_message(chat_id, "Tutorial:\nhttps://t.me/FoXyMx2/867")
        ...
        def handle_text(message):
            ...
            if not numbers:
                self.bot.reply_to(message, "❌ No valid numbers found.")
                return
            ...
            result = "\n".join(normalized)
            self.split_or_file(chat_id, result, "✅ Formatted Numbers:")
        ...
        def handle_doc(message):
            ...
            if not filename.lower().endswith((".txt", ".csv", ".xlsx")):
                self.bot.reply_to(message, "❌ Only txt, csv, xlsx allowed")
                return
            ...
            if not numbers:
                self.bot.reply_to(message, "❌ No numbers found")
                return
            ...
            result = "\n".join(normalized)
            self.split_or_file(chat_id, result, f"✅ Processed {len(normalized)} numbers")
